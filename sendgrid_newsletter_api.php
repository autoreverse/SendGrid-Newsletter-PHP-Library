 ...
 * @author autoreverse https://github.com/autoreverse
 * 
 * Changes 15 Jan 2012: 
 * - renamed file and class to be consistent
...
class SendGrid_Newsletter_API {
    ...
    curl_setopt($session, CURLOPT_RETURNTRANSFER, true);
		...
		// disable SSL peer verification - 15 Jan 2012 autoreverse
    curl_setopt($session, CURLOPT_SSL_VERIFYPEER, FALSE);

		// obtain response
		$jsonResponse = curl_exec($session);
		
		// check for cURL and "other" errors - 15 Jan 2012 autoreverse
    if (!$jsonResponse)
    {
      if ($this->lastResponseError = curl_error($session))
      {
        return false;
      }
      $this->lastResponseError = 'SendGrid API failure. Unknown error';
      return false;
    }
